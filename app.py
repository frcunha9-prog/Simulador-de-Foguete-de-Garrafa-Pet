...
with st.form("params"):
    ...
    with col1:
        V_bottle_L  = st.number_input("Volume da garrafa (L)",
                                      0.3, 5.0, 2.0, 0.1, format="%.2f")
        V_water_L   = st.number_input("Volume de água (L)",
                                      0.05, 4.0, 0.7, 0.05, format="%.2f")
        D_nozzle_mm = st.number_input("Diâmetro do bocal (mm)",
                                      3.0, 30.0, 14.2, 0.1, format="%.1f")
        D_body_mm   = st.number_input("Diâmetro do foguete (mm)",
                                      40.0, 200.0, 104.0, 1.0, format="%.1f")
        m_dry_g     = st.number_input("Massa seca (g)",
                                      30.0, 1000.0, 130.0, 5.0, format="%.1f")
    with col2:
        Cd          = st.number_input("Cd do corpo",
                                      0.1, 1.5, 0.5, 0.05, format="%.2f")
        P0_psi      = st.number_input("Pressão inicial (PSI)",
                                      10.0, 150.0, 87.0, 1.0, format="%.1f")
        Cd_nozzle   = st.number_input("Cd do bocal",
                                      0.6, 1.0, 0.97, 0.01, format="%.2f")
        L_tube_cm   = st.number_input("Comprimento do tubo (cm)",
                                      1.0, 300.0, 100.0, 5.0, format="%.1f")
